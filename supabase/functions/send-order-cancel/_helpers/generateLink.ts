...
export async function generateLink(entry: CancelEntry, contact: string) {
  ...
  const blocks = buildHtmlBlocks(magicLink, entry.order_info, entry.user_observations);
  const htmlContent = renderHtml(entry.template_html, blocks);
  const plainText = renderPlainText(entry.template_html, blocks);

  return {
    from: "SmartiSupply Cancelamento <cancelamento@smartisupply.com.br>",
    to: [email],
    subject: entry.company_name
      ? `${entry.company_name} - Cancelamento de Pedido ${entry.order_info.order_number}`
      : `Cancelamento de Pedido ${entry.order_info.order_number}`,
    html: htmlContent,
    text: plainText,
  };
}
